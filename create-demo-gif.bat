@echo off
echo Creating Web3FuzzForge Demo GIF
...
powershell -Command "Set-Content -Path 'demo-frame1.txt' -Value '# Web3FuzzForge Demo`n`n> Generate ZK-SNARK security tests`n`n$ npm run test:zk-snark`n`nTesting invalid elliptic curve points...`nTesting circuit constraint validations...`nAll tests passed!'"

powershell -Command "Set-Content -Path 'demo-frame2.txt' -Value '# Web3FuzzForge Debug Mode`n`n> Debug cross-chain messaging`n`n$ npm run test:layerzero`n`nDetecting message replay attacks...`nVisualizing cross-chain message flow...`nCreating debug report...'"

powershell -Command "Set-Content -Path 'demo-frame3.txt' -Value '# Web3FuzzForge Security Report`n`n> Vulnerability summary`n`nFound 0 critical issues`nFound 0 high issues`nFound 0 medium issues`n`nAll tests passed! Your dApp is secure.'"
