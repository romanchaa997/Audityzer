/* global describe, it, expect, beforeEach, afterEach, jest */
/**
 * MetaMask Transaction Flow Tests
 * Tests the transaction approval and rejection flow with MetaMask
 */

const { test, expect } = require('@playwright/test');
const path = require('path');

// Path to MetaMask extension directory
const METAMASK_EXTENSION_PATH =
  process.env.METAMASK_EXTENSION_PATH || path.join(__dirname, '../../../extensions/metamask');

// Test configurations
const config = {
  demoAppUrl: 'http://localhost:3000', // Default demo app URL
  extensionUrl: 'chrome-extension://nkbihfbeogaeaoehlefnkodbefgpgknn/home.html',
  testTimeout: 120000, // 2 minutes
};

test.describe('MetaMask Transaction Flow Tests', () => {
  test.beforeEach(async ({ context }) => {
    // Skip test if no extension path available
    if (!process.env.METAMASK_EXTENSION_PATH) {
      test.skip('MetaMask extension path not configured');
    }
  });

  test('should handle transaction approval flow', async ({ browser }) => {
    // Launch browser with MetaMask extension
    const context = await browser.launchPersistentContext('', {
      headless: false,
      args: [
        `--disable-extensions-except=${METAMASK_EXTENSION_PATH}`,
        `--load-extension=${METAMASK_EXTENSION_PATH}`,
      ],
    });

    // Setup MetaMask with test wallet
    const extensionPage = await context.newPage();
    await extensionPage.goto(config.extensionUrl);

    // Wait for MetaMask to load
    await extensionPage
      .waitForSelector('text="Welcome to MetaMask"', { timeout: 10000 })
      .catch(() => {
        console.log('MetaMask already initialized. Continuing with test.');
      });

    // Connect to demo dApp
    const demoPage = await context.newPage();
    await demoPage.goto(config.demoAppUrl);

    try {
      // Connect wallet first
      const connectButton = await demoPage.getByText(/connect|connect wallet|connect metamask/i);
      await connectButton.click();

      // Approve connection in MetaMask popup
      const pages = context.pages();
      let popupPage = pages[pages.length - 1];

      const nextButton = await popupPage.getByRole('button', { name: /next|connect/i });
      if (await nextButton.isVisible()) {
        await nextButton.click();
      }

      const connectButton2 = await popupPage.getByRole('button', { name: /connect|approve/i });
      if (await connectButton2.isVisible()) {
        await connectButton2.click();
      }

      // Wait for connection confirmation
      await demoPage.waitForSelector('text=/connected|account|address/i', { timeout: 10000 });

      // Initiate a transaction (e.g., click a "Send" or "Swap" button)
      const transactionButton = await demoPage.getByText(/send|swap|transfer|mint|submit/i);
      await transactionButton.click();

      // Handle transaction confirmation in MetaMask
      const newPages = context.pages();
      popupPage = newPages[newPages.length - 1];

      // Look for confirmation buttons in popup
      const confirmButton = await popupPage.getByRole('button', {
        name: /confirm|approve|submit/i,
      });
      if (await confirmButton.isVisible()) {
        await confirmButton.click();
      }

      // Verify transaction success in the dApp
      await demoPage.waitForSelector('text=/success|transaction confirmed|completed/i', {
        timeout: 20000,
      });

      // Test passes if no exception thrown
      expect(true).toBeTruthy();
    } catch (error) {
      console.error('Error during transaction flow test:', error);
      expect(false, `Transaction test failed: ${error.message}`).toBeTruthy();
    }

    // Close context
    await context.close();
  });

  test('should handle transaction rejection flow', async ({ browser }) => {
    // Launch browser with MetaMask extension
    const context = await browser.launchPersistentContext('', {
      headless: false,
      args: [
        `--disable-extensions-except=${METAMASK_EXTENSION_PATH}`,
        `--load-extension=${METAMASK_EXTENSION_PATH}`,
      ],
    });

    // Setup MetaMask with test wallet
    const extensionPage = await context.newPage();
    await extensionPage.goto(config.extensionUrl);

    // Connect to demo dApp
    const demoPage = await context.newPage();
    await demoPage.goto(config.demoAppUrl);

    try {
      // Connect wallet first
      const connectButton = await demoPage.getByText(/connect|connect wallet|connect metamask/i);
      await connectButton.click();

      // Approve connection in MetaMask popup
      const pages = context.pages();
      let popupPage = pages[pages.length - 1];

      const nextButton = await popupPage.getByRole('button', { name: /next|connect/i });
      if (await nextButton.isVisible()) {
        await nextButton.click();
      }

      const connectButton2 = await popupPage.getByRole('button', { name: /connect|approve/i });
      if (await connectButton2.isVisible()) {
        await connectButton2.click();
      }

      // Initiate a transaction
      const transactionButton = await demoPage.getByText(/send|swap|transfer|mint|submit/i);
      await transactionButton.click();

      // Reject the transaction in MetaMask
      const newPages = context.pages();
      popupPage = newPages[newPages.length - 1];

      // Look for reject button in popup
      const rejectButton = await popupPage.getByRole('button', { name: /reject|cancel|decline/i });
      if (await rejectButton.isVisible()) {
        await rejectButton.click();
      }

      // Verify transaction rejection in the dApp (may show an error or return to initial state)
      await demoPage
        .waitForSelector('text=/rejected|cancelled|failed|error/i', { timeout: 10000 })
        .catch(() => {
          // If no explicit rejection message, check we're back at the initial state
          return demoPage.waitForSelector('text=/send|swap|transfer|mint|submit/i', {
            timeout: 10000,
          });
        });

      // Test passes if no exception thrown
      expect(true).toBeTruthy();
    } catch (error) {
      console.error('Error during transaction rejection test:', error);
      expect(false, `Transaction rejection test failed: ${error.message}`).toBeTruthy();
    }

    // Close context
    await context.close();
  });
});
