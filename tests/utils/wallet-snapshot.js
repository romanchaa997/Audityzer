    ...
    await page.evaluate(({ address, chainId }) => {
      window.ethereum = {
        ...
        removeListener: (eventName, callback) => {
          ...
          if (window.ethereum._events[eventName]) {
            window.ethereum._events[eventName] = window.ethereum._events[eventName]
      ...
      function updateUI() {
        if (document.getElementById('wallet-info')) {
          document.getElementById('wallet-info').style.display = 
            window.ethereum.selectedAddress ? 'block' : 'none';
        }
  ...
  try {
    ...
    while (attempts < maxAttempts) {
      const isVisible = await page.evaluate(() => {
        ...
        return walletInfo.style.display !== 'none' && 
               style.display !== 'none' && 
