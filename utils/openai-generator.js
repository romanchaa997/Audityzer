  ...
  try {
    ...
    const response = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: 'system',
          content: `You are a specialized AI for creating Audityzer test configurations. 
...
/**
 * Save the generated configuration to .audityzer.json
 * @param {Object} config - Configuration object to save
 * @returns {string} Path to the saved config file
 */
async function saveGeneratedConfig(config) {
  try {
    const configPath = path.join(process.cwd(), '.audityzer.json');
